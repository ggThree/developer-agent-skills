...
`release_check.sh` 默认模式在 blocker 或 unknown 存在时返回非零；只有普通 warning 时可能以退出码 `0` 返回“有条件允许”。无人值守 CI 必须拒绝任何非零退出、保存 warning 报告，并继续执行项目自己的构建、测试、签名和部署 required checks。`--strict` 会把所有 warning 一并阻断，只适合明确采用零 warning 政策的流水线，不能替代这些项目证据。
